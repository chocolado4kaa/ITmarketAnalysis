  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden' }}>
      <div style={{
        ...
        opacity: 0.06, fontFamily: "'Playfair Display', serif",
        fontSize: 200, fontWeight: 700, pointerEvents: 'none',
        userSelect: 'none', color: 'var(--accent)', lineHeight: 1,
      }}>IT</div>
        ...
        Сучасні реалії{' '}
        <em style={{ fontStyle: 'italic', color: 'var(--accent)' }}>IT-ринку</em>
      </h1>

      <p style={{ fontSize: 17, color: 'var(--muted)', maxWidth: 520, lineHeight: 1.6, marginBottom: 48 }}>
        Що насправді відбувається у сфері розробки програмного забезпечення — і як це впливає на вас як на майбутніх спеціалістів.
      </p>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        {chips.map(({ label, idx }) => (
          <Chip key={idx} onClick={() => onGoTo(idx)}>{label}</Chip>
        ))}
      </div>
    </div>
  );
